...
from config.balance import ENEMY_COLOR
...
def _update_wave_timers(state, dt: float, ctx: dict) -> None:
    ...
    # Use custom map spawning if a custom map is active
    custom_map = getattr(state, "custom_map", None)
    if custom_map is not None:
        start_wave_from_map(state.wave_number, state, custom_map, ctx)
    else:
...
# CUSTOM MAP SPAWNING
# =============================================================================
# Map spawning has been extracted to systems/spawning/map_spawn.py
# Re-export for backward compatibility
from .spawning.map_spawn import spawn_enemies_from_map, start_wave_from_map
