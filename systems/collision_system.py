...
Delegates to collision_player, the collision package, and collision_pickups.
...
def update(state: "GameState", dt: float) -> None:
    ...
    with perf_timer("collision_system"):
        ...
        block_grid = collision.build_block_grid_cached(state, ctx)
        ctx["_block_grid"] = block_grid

        collision.handle_hazard_enemy_collisions(state, dt, ctx)
        collision.handle_laser_beam_collisions(state, dt, ctx)
        collision_player.handle_enemy_laser_beam_collisions(state, dt, ctx)
        collision.handle_dead_enemies(state, ctx)
        collision.handle_player_bullet_offscreen(state, ctx)
        collision.handle_player_bullet_enemy_collisions(state, ctx)
        collision.handle_player_bullet_block_collisions(state, dt, ctx)
        collision.handle_enemy_projectile_lifetime_offscreen(state, ctx)
        collision.handle_enemy_projectile_block_collisions(state, ctx)
        collision_player.handle_enemy_projectile_player_collisions(state, ctx)
        collision.handle_enemy_projectile_friendly_collisions(state, ctx)
        collision.handle_enemy_projectile_decoy_collisions(state, ctx)
        ...
        collision.handle_friendly_projectile_offscreen_blocks_enemies(state, ctx)
        collision.handle_grenade_explosion_damage(state, dt, ctx)
        collision.handle_missile_collisions(state, ctx)
