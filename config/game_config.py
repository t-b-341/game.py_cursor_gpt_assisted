...
- Lightweight CPU effects (no GPU shaders): enable_menu_shaders + menu_effect_profile ("crt" | "soft_glow" | "grainy" | "swirly"), enable_gameplay_shaders + gameplay_effect_profile ("subtle_vignette" | "crt_light" | "film_grain" | "atmospheric"), enable_damage_wobble (see visual_effects).
...
class GameConfig:
    ...
    # Lightweight CPU effect toggles and profiles (visual_effects module; no GPU shaders)
    enable_menu_shaders: bool = False  # Apply effect stack to title/options/pause (scanlines, vignette, tint, grain).
    enable_gameplay_shaders: bool = False  # Apply vignette/scanlines/grain/low-HP pulse to gameplay.
    menu_effect_profile: str = "none"  # "none" | "crt" | "soft_glow" | "grainy" | "swirly"
    gameplay_effect_profile: str = "none"  # "none" | "subtle_vignette" | "crt_light" | "film_grain" | "atmospheric"
    enable_damage_wobble: bool = False  # Brief screen jitter when player takes damage (eased decay).
    ...
    # NOTE: despite the name, in the gameplay render path this only enables CPU muzzle-flash
    # and rocket-glow effects (_apply_gpu_visual_effects). The GPU ShaderPipelineManager is
    # populated by apply_shader_settings_to_pipeline() but only executed by the shader test
    # and settings-preview scenes. See AGENTS.md.
    use_gpu_shader_pipeline: bool = False
