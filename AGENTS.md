...
Module‑layout caveat: **`enemies.py` (root) is a compat re‑export** of the `enemies/` package
(`ai.py`, `factory.py`, `movement.py`). New code should import from the package.
...
- **Keep backward‑compat shims working** if you touch a module that re‑exports from a package
  (see §10) — or update all call sites.
...
## 10. Code‑review hotspots (verify these carefully)

These are known rough edges surfaced during a codebase survey. They are *starting points for
scrutiny*, not confirmed bugs — verify against the current tree before acting.
...
- `systems/collision/` subpackage (`helpers/hazards/player_bullets/enemy_projectiles/
  friendly_projectiles/explosions.py`) looks like an **unwired parallel refactor**; the live path
  uses monolithic `collision_projectiles.py` + `collision_player/pickups/common/movement.py`.
  Confirm which one a change should touch (and whether tests reference the other).
- Compat re‑export shims: `enemies.py`→`enemies/`, `rendering/world.py`→`rendering/world/`,
  `rendering_shaders.py`→`rendering/shaders/pipeline.py`, `systems/ui_system.py`→`systems/ui/core.py`,
  `maps/editor.py`→`maps/editor/`, `scenes/shader_settings.py`→split modules.
- `enemies/movement.py:move_enemy_with_push_cached` appears superseded by
  `collision_movement.move_enemy_with_push` (wired via `level_context`).
- Two CPU effect systems (`visual_effects.py` vs `shader_effects/`) with overlapping profiles;
  they can stack unintentionally. `apply_pause_effects()` reportedly checks `enable_menu_shaders`
  rather than `enable_pause_shaders`.
...
**Telemetry/ML:**
- Several `log_*` methods and wave `"end"` events have schema/writer support but **no live call
  sites**, so some plots may show "no data". `map_spawn.py` reportedly calls a non‑existent
  `log_wave_start` inside a swallowed `try/except`.
- `wave_summaries` (DDA training data) are only produced when `enable_dda` is on; `ml/ab_test.py`
  enables the DDA singleton but may not set `config.enable_dda`, so the spawn hook can stay off.
...
1. Is the change in the **live path** or a parallel/legacy module (see §10)? Grep for call sites.
