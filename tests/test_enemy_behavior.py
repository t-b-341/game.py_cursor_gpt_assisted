...
# --- clamp_rect_to_screen tests ---

def test_clamp_stays_in_bounds_when_already_inside():
    r = pygame.Rect(10, 20, 30, 40)
    clamp_rect_to_screen(r, 800, 600)
    assert r.x == 10 and r.y == 20


def test_clamp_left_and_top():
    r = pygame.Rect(-5, -10, 30, 40)
    clamp_rect_to_screen(r, 800, 600)
    assert r.x == 0 and r.y == 0


def test_clamp_right_and_bottom():
    r = pygame.Rect(780, 590, 30, 40)
    clamp_rect_to_screen(r, 800, 600)
    assert r.x == 770 and r.y == 560


# --- find_nearest_threat tests ---

def test_find_nearest_threat_none_when_no_player_and_no_friendlies():
    pos = pygame.Vector2(100, 100)
    assert find_nearest_threat(pos, None, []) is None


def test_find_nearest_threat_player_when_player_present():
    pos = pygame.Vector2(100, 100)
...
def test_find_nearest_threat_nearest_friendly_when_no_player():
    pos = pygame.Vector2(100, 100)
...
def test_find_nearest_threat_dropped_ally_priority_within_radius():
    pos = pygame.Vector2(100, 100)
...
def test_find_nearest_threat_skips_dead_friendlies():
    pos = pygame.Vector2(100, 100)
    dead = {"rect": pygame.Rect(110, 110, 24, 24), "hp": 0}
    assert find_nearest_threat(pos, None, [dead]) is None


# --- make_enemy_from_template tests ---

def test_pawn_template_returns_enemy_with_expected_fields():
...
def test_enemy_supports_dict_like_access():
    t = next(tmpl for tmpl in ENEMY_TEMPLATES if tmpl.get("type") == "grunt")
    e = make_enemy_from_template(t, 1.0, 1.0)
    assert e["type"] == "grunt"
